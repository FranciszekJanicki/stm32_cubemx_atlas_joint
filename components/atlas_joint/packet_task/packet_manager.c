                                                  ...
                                                  atlas_hmi_packet_t const* packet)
{
    ...
    return true;
    // HAL_SPI_Transmit(manager->config.packet_spi, (uint8_t*)packet, sizeof(*packet), 100) ==
    // HAL_OK;
}
                                                     ...
                                                     atlas_rob_packet_t* packet)
{
    ...
    return true;
    // HAL_SPI_Receive(manager->config.packet_spi, (uint8_t*)packet, sizeof(*packet), 100) ==
    // HAL_OK;
}
...
static inline void packet_manager_set_hmi_packet_ready_pin(packet_manager_t* manager, bool state)
{
    ...
    HAL_GPIO_WritePin(manager->config.hmi_packet_ready_gpio,
                      manager->config.hmi_packet_ready_pin,
                      (GPIO_PinState)state);
}
